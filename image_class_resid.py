...
def get_pools(pool_filters: List[Tuple[int, int, str]]) -> List[jnp.ndarray]:
    pools_list = [x_train[:train_n]] + [jax.vmap(apply_pooling, axes=(0, None))(x_train[:train_n], pool_filter) for pool_filter in pool_filters]
    pools_test = [x_test[:test_n]] + [jax.vmap(apply_pooling, axes=(0, None))(x_test[:test_n], pool_filter) for pool_filter in pool_filters]
